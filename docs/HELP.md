...
扫码创建或选择已有应用时，会打开带应用模板的飞书确认页，由飞书流程补齐群聊消息、发送消息、读取附件、长连接事件和卡片回调能力。
...
- 如果主机器人聊天能收到、独立群聊收不到，优先重新扫码选择已有应用，让飞书确认页按模板补齐群聊消息事件。
...
重新扫码并选择已有应用，让飞书确认页按模板补齐机器人权限、长连接消息事件和卡片回调。服务日志会输出 reply bridge 的启动和路由错误。
