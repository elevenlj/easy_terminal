...
扫码创建应用时会请求群聊消息、发送消息、创建群聊、读取附件和卡片回调能力。若飞书后台没有自动带上，请在应用的“事件与回调”里确认长连接已启用 `im.message.receive_v1`，卡片回调已启用 `card.action.trigger`。
...
- 如果主机器人聊天能收到、独立群聊收不到，优先检查群聊消息权限和 `im.message.receive_v1` 长连接事件是否已启用。
...
确认飞书应用启用了机器人能力、消息事件订阅和必要权限。群聊消息需要 `im.message.receive_v1` 长连接事件，卡片按钮需要 `card.action.trigger` 回调。服务日志会输出 reply bridge 的启动和路由错误。
