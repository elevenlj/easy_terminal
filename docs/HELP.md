...
- 后端会维护终端屏幕快照，用于生成飞书等待通知；页面打开与否不影响通知正文的基础数据源。

## 飞书联动

配置飞书应用后，服务会通过飞书 WebSocket 事件接收消息，并把消息路由到对应终端会话。

扫码创建或选择已有应用时，会打开带应用模板的飞书确认页，由飞书流程补齐基础消息、发送消息、读取附件、长连接事件和卡片回调能力。

如果希望在会话群里不 @ 机器人也能响应，需要在飞书后台为当前应用开通“获取群组中所有消息（敏感权限）”。该权限对应 `im:message.group_msg`，扫码创建应用不一定会自动开通，通常需要发布应用版本并通过管理员审批。
...
- `开始 会话名`、`新会话 会话名`、`/new 会话名`：创建新终端会话；未命中会话名预设时，进入默认工作目录并启动“默认会话 Agent”。
- `开始`、`新会话`、`/new`：使用 `lark_default_session_name` 作为默认会话名。
- `开始 会话名 0`：只创建会话并进入默认工作目录，不启动 Agent。
- `开始 会话名 999999`：创建会话并显式启动“默认会话 Agent”。
...
- 如果群里 @ 机器人能收到、普通群消息收不到，优先检查飞书后台是否已开通并发布“获取群组中所有消息（敏感权限）”。
...
- 优先使用后端维护的终端屏幕快照。
- 浏览器页面打开时仍会同步可见快照作为辅助，但飞书通知不依赖页面在线。
- 每次提交输入时，会记录输入文本和输入前的屏幕快照；推送时优先从本次输入回显之后截取当前轮回复。
- 找不到输入锚点时，使用本轮原始输出兜底。
...
| `CHROME_BIN` | 自动查找 | 可选的 headless 浏览器路径 |
...
先点击“测试飞书配置”。如果“群所有消息权限”失败，需要在飞书后台为当前 App 开通“获取群组中所有消息（敏感权限）”，发布/审批后再保存配置。服务日志会输出 reply bridge 的启动和路由错误。

### 飞书没有创建独立会话群聊

确认飞书应用已开通创建群、读取/接收群消息、发送群消息等 IM 权限，并且应用机器人可被拉入群聊。创建失败时服务会在日志里记录 `failed to create session chat`，同时回退到单机器人聊天模式。

### 通知内容不完整

Codex/TUI 程序会由后端终端屏幕模型生成通知正文，不需要依赖浏览器页面保持打开。若本机 TUI 使用了特别复杂的终端控制序列，打开 Web 会话仍可作为辅助校准。

### 快照出现终端噪声

服务会过滤浏览器/xterm 自动回写的终端能力响应，并由后端屏幕模型处理常见 ANSI 重绘。若仍出现异常，优先确认对应 TUI 是否使用了特殊终端控制序列。
